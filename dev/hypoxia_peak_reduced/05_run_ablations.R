...
ablation_root <- file.path(out_dir, "ablations")
dir.create(ablation_root, recursive = TRUE, showWarnings = FALSE)

prepared <- readRDS(file.path(out_dir, "prepared_input.rds"))
ablation_registry <- get_ablation_registry()

run_parts_list <- lapply(ablation_registry, function(spec) {
  cat("Running ablation:", spec$name, "\n")
  run_parts <- run_ablation_fit(project_root = project_root, prepared = prepared, spec = spec, iter = iter)
  write_fit_outputs(run_parts)
  render_fit_outputs(run_parts$out_dir)
  run_parts
})
names(run_parts_list) <- names(ablation_registry)

comparison_tbl <- bind_rows(lapply(run_parts_list, summarise_ablation_run))
write.csv(comparison_tbl, file.path(ablation_root, "ablation_comparison.csv"), row.names = FALSE)

cat("Saved ablation outputs under:", ablation_root, "\n")
