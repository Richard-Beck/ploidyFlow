...
parameters {
  ...
  // Global load-response coefficient. beta = 0 recovers the first-pass model;
  // larger values pull tumor peaks left in samples with more tumor DNA
  // relative to the centering/control population.
  real<lower=0> beta;
...
transformed parameters {
  ...
  for (s in 1:N_sample) {
    ...
    load_proxy[s] = log((2 * pop_theta[s][2] + 4 * pop_theta[s][3]) / pop_theta[s][1]);
    ratio_tumor_cen_sample[s] = 1 + min_tumor_cen_ratio_gap + exp(log_ratio_tumor_cen_mu - beta * load_proxy[s]);
