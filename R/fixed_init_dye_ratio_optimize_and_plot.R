...
exclude_control_samples_from_fit <- FALSE
excluded_control_sample_patterns <- c("2N_Control", "4N_Control")
run_label <- if (exclude_control_samples_from_fit) {
  sprintf("%s_exclude_2n_4n_controls", experiment_filter)
} else {
  experiment_filter
}
fit_root <- file.path(project_root, "processed_data", sprintf("stan_optimize_fixed_init_dye_ratio_%s", run_label))
plot_dir <- file.path(project_root, "figure", sprintf("stan_optimize_fixed_init_dye_ratio_%s", run_label))
exe_file <- file.path(fit_root, "ploidy_histogram_dye_ratio_fixed_init.exe")
init_plot_path <- file.path(plot_dir, "fixed_init_dye_ratio_overlay.png")
opt_plot_path <- file.path(plot_dir, "fixed_init_dye_ratio_optimized_overlay.png")
output_basename <- "fixed_init_dye_ratio_optimize"
manifest_path <- file.path(fit_root, "fixed_init_dye_ratio_optimize_manifest.rds")
...
exclude_samples_from_stan_bundle <- function(stan_data, lut, sample_patterns) {
  ...
  if (!any(exclude_samples)) {
    warning(sprintf(
      "No samples matched exclusion patterns: %s",
      paste(sample_patterns, collapse = ", ")
    ))
    return(list(stan_data = stan_data, lut = lut, excluded_samples = lut$sample[FALSE, , drop = FALSE]))
  }
  ...
  if (!any(keep_samples)) {
    stop("Sample exclusion removed all samples.")
  }
...
if (exclude_control_samples_from_fit) {
  filtered_bundle <- exclude_samples_from_stan_bundle(
    stan_data = stan_data,
    lut = lut,
    sample_patterns = excluded_control_sample_patterns
  )
  stan_data <- filtered_bundle$stan_data
  lut <- filtered_bundle$lut
  excluded_samples <- filtered_bundle$excluded_samples
}
...
load_proxy_init <- compute_load_proxy(fixed_init$pop_theta)
ratio_tumor_cen_sample_init <- 1 + min_tumor_cen_ratio_gap +
  exp(fixed_init$log_ratio_tumor_cen_mu - fixed_init$beta * load_proxy_init)
...
mod <- cmdstan_model(stan_file, compile = TRUE, exe_file = exe_file, force_recompile = FALSE)
...
manifest <- list(
  ...
  exclude_control_samples_from_fit = exclude_control_samples_from_fit,
  excluded_control_sample_patterns = excluded_control_sample_patterns,
  excluded_samples = excluded_samples,
